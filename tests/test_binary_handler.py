...
def test_handle_hexdump_not_found(tmp_path: Path) -> None:
    ...
    with patch("shutil.which", return_value=None), patch("l_command.handlers.binary.logger") as mock_logger:
        BinaryHandler.handle(binary_file)

        # Verify error was logged
        mock_logger.error.assert_called_once_with("Error: 'hexdump' command not found. Cannot display binary file.")
